...
class Index extends Component
{
    ...
    public $perPage = '10';
    public $updown = 'asc';
    public $str_filter = 'management.id';
    public $total = 0;
    public $time = 0;
    public $confirming = 0;
    public $item_id = 0;
    ...
    public function render()
    {
        ...
        $management = Management::search("%{$this->search}%")
            ->orderBy($this->str_filter, $this->updown)
            ->paginate($this->perPage);

        $this->parents = Management::all();

        $end_time = microtime(true);
        $this->time = $end_time - $start_time;

        return view('livewire.management.index', ['management' => $management]);
    }
    ...
    public function save()
    {
        if ($this->item_id) {
            ...
            $data = Management::find($this->item_id);
            $message = "Actualizado";
        } else {
            $this->validate([
                'parent_id' => ['numeric'],
